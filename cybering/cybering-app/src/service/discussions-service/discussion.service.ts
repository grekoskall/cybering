...
import { Discussion, DiscussionReply, DiscussionsInfo } from 'src/app/interfaces/discussion';
...
export class DiscussionService extends BaseService {
  ...
  getDiscussionsArrayWithParam(
    cookie: string,
    profid: string): Observable<DiscussionsInfo> {
    let profidFromUrl = new SimpleString(profid);
    return this.http.post<DiscussionsInfo>(
      this.extendurl('cybering/discussions'),
      profidFromUrl,
      {
        headers: {
          "Cookies": cookie,
          "action": "discussion-array-with-param"
        }
      }
    ).pipe(
      retry(2),
